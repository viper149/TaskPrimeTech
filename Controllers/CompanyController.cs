...
namespace PrimeTechApp.Controllers
{
    public class CompanyController : Controller
    {
        ...
        public async Task<ActionResult> Create(Company company)
        {
            try
            {
                return (await _company.Create(company)) ? RedirectToAction(nameof(Index)) : RedirectToAction(nameof(Create));
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex);
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (exCom != null)
                    {
                        ...
                        if (await _company.Update(exCom))
                        {
